...
class EmbeddingService:
    def __init__(self):
        ...
        engine_args = []
        for model_name, model_display_name, batch_size, dtype in zip(
            self.config.model_names, self.config.models_display_names, self.config.batch_sizes, self.config.dtypes
        ):
            engine_args.append(
                EngineArgs(
                    ...
                    served_model_name=model_display_name,
                )
            )
    ...
    async def route_openai_models(self) -> OpenAIModelInfo:
        return OpenAIModelInfo(data=[ModelInfo(id=model_id, stats={}) for model_id in self.list_models()]).model_dump()
    ...
    async def infinity_rerank(self, query: str, docs: str, return_docs: str, model_name: str):
        """Rerank the documents based on the query"""
        if not self.is_running:
            await self.start()
        scores, usage = await self.engine_array[model_name].rerank(query=query, docs=docs, raw_scores=False)
        if not return_docs:
            docs = None
        return to_rerank_response(scores=scores, documents=docs, model=model_name, usage=usage)
